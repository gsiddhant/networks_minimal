...
MultiLayerPerceptron::MultiLayerPerceptron(const std::vector<unsigned int> &networkLayers,
                                           const std::vector <std::reference_wrapper<Activation>> &networkActivations,
                                           const std::string &networkParametersPath)
        : MultiLayerPerceptron::MultiLayerPerceptron(networkLayers, networkActivations) {
    loadNetworkParametersFromFile(networkParametersPath);
}

void MultiLayerPerceptron::initializeNetworkVariables() {
...
void MultiLayerPerceptron::loadNetworkParametersFromFile(const std::string &networkParametersPath) {
...
void MultiLayerPerceptron::resetNetworkParameters() {
...
const Eigen::MatrixXd &MultiLayerPerceptron::forward(const Eigen::MatrixXd &networkInput) {
    ...
    if (outputActivation_)
        latentOutput_.back() = networkActivations_[networkActivations_.size() - 1].get().forward(latentOutput_.back());

    return latentOutput_.back();
}

const Eigen::MatrixXd &MultiLayerPerceptron::gradient(const Eigen::MatrixXd &networkInput) {
    latentOutput_[0] = networkInput;
    ...
    for (int layer = static_cast<int>(networkActivations_.size() - (outputActivation_ ? 2 : 1)); layer >= 0; --layer) {
